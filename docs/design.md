...
`{colors.brand}`(밝은 그린)는 흰 텍스트 대비가 아슬아슬하므로, 흰 텍스트를 얹는 CTA에는 한 단계 진한 `{colors.bg-brand}`를 쓴다. 강조 텍스트는 `{colors.fg-brand}`(green-800)로 더 진하게 쓴다. **`{colors.brand}`는 보더·트랙·인디케이터 등 비-텍스트 표면 전용** — 흰 배경 위 단독 텍스트로는 형광에 가까워 부담스럽다. 텍스트에 그린을 쓸 때는 outlined 버튼처럼 절제된 강조면 `{colors.bg-brand}`, 옅은 tint 위 더 진한 강조면 `{colors.fg-brand}`를 쓴다.
...
버튼은 사이즈에 따라 8~14로 스케일한다(xSmall 8 → xLarge 14). 칩은 완전한 pill.
...
주 행동 버튼. 5사이즈, radius가 사이즈 따라 스케일.

| 사이즈 | height | radius | typography       |
| ------ | ------ | ------ | ---------------- |
| xLarge | 56     | 14     | title-2 (16/600) |
| large  | 48     | 12     | title-2 (16/600) |
...
| outlined  | 흰색 + 1px `{colors.line-default}` | `{colors.bg-brand}`   |

- **Disabled**: 불투명도 변화 없이 색 교체 — bg `{colors.gray-100}` / text `{colors.fg-disabled}`.
- **Loading**: variant 색·형태 유지 + 스피너, 클릭 차단, 너비 유지.
...
배경 없음, radius 4 고정, padding 4/2. variant로 색 결정(primary `{colors.bg-brand}` / secondary `{colors.fg-default}` / tertiary `{colors.fg-secondary}`). 옵션 underline.
...
선택 가능한 pill(radius `{rounded.radius-full}`), padding 8/12, gap 6. medium/small/xSmall(높이 32~36).
...
- **Active**: `{motion.scale-press-row}` + 리플. **Focus(웹)**: 3px ring 50% — 전 인터랙티브 컴포넌트 공통 focus-visible 규격.
...
`<label>`(large body-2 / medium body-3, `{colors.fg-secondary}`) + 필드(기본 medium 40, 모바일 large 48) + helper(body-3 `{colors.fg-tertiary}`).

| variant   | 표면                                   | radius |
| --------- | -------------------------------------- | ------ |
| filled    | `{colors.gray-100}` bg + 1px 투명 보더 | 14     |
| outlined  | 흰 bg + 1px `{colors.line-default}`    | 14     |
| underline | 하단 보더만(평소 `{colors.line-default}`) | 0      |

- **Focus**: 보더 `{colors.line-brand}`. **Error**: 보더 + helper `{colors.fg-danger}`. **Disabled**: bg `{colors.gray-100}` / text `{colors.fg-disabled}`.

### text-area

멀티라인. 최소 64(auto-grow), gap 12. 글자수 카운터 body-3 `{colors.fg-tertiary}` "0/200". variant는 input과 동형(filled/outlined/underline).
...
sticky 바 1px 하단 보더 `{colors.line-default}` + px 16. 탭 38 높이 title-2. active `{colors.fg-default}` / inactive `{colors.fg-secondary}`, press `{motion.scale-press-row}`. active 탭 하단 2px 인디케이터.
...
- **Modal/BottomSheet 딤 배경**은 `ui/dialog.tsx`·`ui/sheet.tsx`가 `bg-black/50`을 하드코딩해 `overlay-scrim` 토큰과 값이 다르다. `ui/` 직접 수정 금지 규칙 때문에 core 래퍼에서 고칠 수 없는 알려진 제약이다.
