...
const meta: Meta<typeof ComponentName> = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: '핵심 설명 한 문장 — 역할·사용법.\n\n디자인 스펙 문단 — 수치·색·radius 등.',
      },
    },
  },
  tags: ['autodocs'],
...
### 컴포넌트 설명(description.component) 포맷

한 줄로 길게 잇지 말고 `\n\n`으로 **두 문단**을 나눈다 (docs는 마크다운 렌더 — `\n\n`이 문단 구분):

1. **1문단 — 핵심 설명**: 무엇인지 + 사용법·API 특이사항 (예: "체크박스 — label을 지정하면 높이 48 행 레이아웃이 되고 행 press 피드백이 붙는다.")
2. **2문단 — 디자인 설명**: 수치·색·radius 등 시각 스펙 (예: "20×20 원형 컨트롤. 체크 시 브랜드 배경 + 흰 체크, disabled는 불투명도 대신 gray-300으로 색 강등된다.")
